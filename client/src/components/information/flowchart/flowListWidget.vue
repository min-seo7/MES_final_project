...
const emit = defineEmits(['flowchartSelected']);

// row 클릭 시
const onRowClick = (event) => {
    emit('flowchartSelected', event.data);
};
...
<template>
    <DataTable :value="tableData" :rows="rowsPerPage" :paginator="true" :totalRecords="props.items.length" showGridlines @row-click="onRowClick" @page="onPage" selection-mode="single" responsiveLayout="scroll">
        ...
        <Column field="flowName" header="흐름도명" style="width: 150px" />
        ...
        <Column field="createDate" header="등록일자" style="width: 150px" />
        <Column field="status" header="상태" style="width: 150px" />
    </DataTable>
</template>
