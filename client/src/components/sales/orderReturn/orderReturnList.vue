...
const searchOrders = async () => {
  try {
    const startDate = returnDateRange.value.startDate
      ? formatDate(returnDateRange.value.startDate)
      : null;
    const endDate = returnDateRange.value.endDate
      ? formatDate(returnDateRange.value.endDate)
      : null;
    const reStatusInt =
      ship.value === "반품등록" ? 1 : ship.value === "반품완료" ? 2 : null;
...
<template>
  <div>
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">반품조회</h1>
        ...
        <div class="flex flex-wrap gap-6 p-4">
          ...
          <div class="flex flex-col">
            ...
            <div class="flex items-center gap-2">
              <Calendar
                v-model="returnDateRange.startDate"
                dateFormat="yy-mm-dd"
                showIcon
                class="w-full"
              />
              <span>~</span>
              <Calendar
                v-model="returnDateRange.endDate"
                dateFormat="yy-mm-dd"
                showIcon
                class="w-full"
              />
    ...
    <DataTable :value="customers2" paginator :rows="5" class="mt-6">
      ...
      <Column field="reState" header="반품상태" style="min-width: 100px">
        <template #body="slotProps">
          <Tag
            :value="slotProps.data.reState"
            :severity="getSeverity(slotProps.data.reState)"
            :rounded="true"
      ...
      <Column field="returnReason" header="반품사유" style="min-width: 100px">
        <template #body="slotProps">
          <span :title="slotProps.data.returnReason">{{
