...
const selectSupplierAndClose = () => {
  if (selectedSupplierFromDialog.value) {
    search.value.partnerId = selectedSupplierFromDialog.value.partnerId;
  }
...
const selectProductAndClose = () => {
  if (selectedProductFromDialog.value) {
    ...
    search.value.spec = selectedProductFromDialog.value.specification; // 제품 선택 시 규격도 함께 설정
  }
...
const fetchOrders = async () => {
  try {
    ...
    let formattedDelDate = delDateValue
      ? `${delDateValue.getFullYear()}-${String(
          delDateValue.getMonth() + 1
        ).padStart(2, "0")}-${String(delDateValue.getDate()).padStart(2, "0")}`
      : null;
    ...
    const response = await axios.get("/api/sales/pdfEmail", {
      params: queryParams,
    });
    ...
    if (response.data?.list && Array.isArray(response.data.list)) {
      ...
      totalRecords.value = response.data.totalCount || 0;
    } else {
      orders.value = [];
...
const pdfForm = computed(() => {
  ...
  return {
    fileName: `${(d.orderDate || "").replace(/-/g, "")}_${
      d.partnerName || ""
    }_${d.orderId || ""}_주문서.pdf`,
...
const savePdf = async () => {
  ...
  try {
    const response = await axios.get(`/api/sales/pdf/download/${orderId}`, {
      responseType: "blob",
    });
...
<template>
  <div>
    ...
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">PDF/이메일</h1>
      ...
      <template #center>
        <div class="flex flex-wrap gap-6 p-4">
          <div class="flex flex-col">
            <label for="orderId" class="font-semibold text-sm mb-1"
              >주문번호</label
            >
            <InputGroup>
      ...
      <Panel header="이메일">
        <div class="grid grid-cols-2 gap-4">
          <div class="flex items-center space-x-2">
            ...
            <InputText
              v-model="emailForm.partnerEmail"
              readonly
              class="flex-grow"
            />
          </div>
          <div class="flex items-center space-x-2">
            ...
            <InputText
              v-model="emailForm.managerEmail"
              readonly
              class="flex-grow"
            />
          </div>
        ...
        <template #footer>
          <div class="flex justify-end space-x-2">
            <Button
              ...
              @click="sendEmail"
              :disabled="
                selectedOrders.length === 0 || selectedOrders.length > 1
              "
            />
            <Button
              label="초기화"
    ...
    <Dialog
      v-model:visible="showOrdersModal"
      modal
      header="주문서 목록"
      :style="{ width: '50vw' }"
      class="centered-dialog"
    >
      <div class="p-4">
        <DataTable
          ...
          selectionMode="single"
          dataKey="orderId"
          v-model:selection="selectedOrdersFromDialog"
          :rowHover="true"
          :paginator="true"
          :rows="5"
        >
