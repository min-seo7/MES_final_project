<!-- src/components/equipment/repair/repairSearchWidget.vue -->
<script setup>
import { ref } from 'vue';
...
// 조회 버튼
const onSubmit = () => {
    // ✅ 서버/리스트위젯에서 기대하는 키로 변환하여 전달
    emit('submit', { repair_id: search.value.repairCode });
};
// 초기화 버튼
const onClear = () => {
...
<template>
    <div class="space-y-2">
        ...
        <Dialog v-model:visible="showPicker" header="수리코드 선택" :modal="true" :style="{ width: '30rem' }">
            <ul class="divide-y">
                <li v-for="(code, idx) in pickerList" :key="idx" class="p-2 flex items-center gap-3 cursor-pointer" @click="selected = code">
                    <RadioButton :inputId="`repair_${idx}`" name="repair_pick" :value="code" v-model="selected" />
