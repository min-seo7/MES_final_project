...
<template>
  <div class="mt-8 space-y-4">
    <div class="flex items-center justify-between">
      <div class="font-bold text-[18.5px]"></div>
    ...
    <div class="border rounded-md overflow-hidden bg-white">
      <div class="grid grid-cols-[9rem_1fr_9rem_1fr]">
        <!-- 설비코드 / 설비명 -->
        <div
          class="bg-gray-50 border-r border-b px-2 py-4 text-[14px] font-medium flex items-center justify-center whitespace-nowrap"
        >
        ...
        <div
          class="bg-gray-50 border-r border-b px-2 py-4 text-[14px] font-medium flex items-center justify-center whitespace-nowrap"
        >
          <span class="text-red-500 mr-1">*</span>설비명
        </div>
        ...
        <div
          class="bg-gray-50 border-r border-b px-2 py-4 text-[14px] font-medium flex items-center justify-center whitespace-nowrap"
        >
          <span class="text-red-500 mr-1">*</span>제조사
        </div>
        ...
        <div
          class="bg-gray-50 border-r border-b px-2 py-4 text-[14px] font-medium flex items-center justify-center whitespace-nowrap"
        >
          <span class="text-red-500 mr-1">*</span>구매일
        </div>
        <div class="border-r border-b px-3 py-4 flex items-center">
          <Calendar
        ...
        <div
          class="bg-gray-50 border-r border-b px-2 py-4 text-[14px] font-medium flex items-center justify-center whitespace-nowrap"
        >
          <span class="text-red-500 mr-1">*</span>설비유형
        </div>
        ...
        <div class="border-b px-3 py-5 col-span-3">
          ...
          <div class="text-right text-xs text-gray-500 mt-1">
            {{ form.note?.length || 0 }}/200
          </div>
        </div>

        <!-- 파일첨부 (그대로) -->
        <div
          class="bg-gray-50 border-r px-2 py-4 text-[14px] font-medium flex items-center justify-center whitespace-nowrap"
        >
          파일첨부
        </div>
        <div class="px-4 py-6 col-span-3 space-y-6">
          <div class="flex items-center gap-4">
            ...
            <button type="button" class="flex-1" @click="pickFile('file1')">
              <div
                class="w-full h-12 rounded-full border-2 border-dashed border-blue-400 flex items-center justify-center"
              >
                <span
                  class="truncate px-4 text-base"
                  :class="file1Name ? 'text-gray-700' : 'text-gray-400'"
                >
                  {{ file1Name || "파일을 선택하세요." }}
                </span>
              </div>
          ...
          <div class="flex items-center gap-4">
            ...
            <button type="button" class="flex-1" @click="pickFile('file2')">
              ...
              >
                <span
                  class="truncate px-4 text-base"
                  :class="file2Name ? 'text-gray-700' : 'text-gray-400'"
                >
                  {{ file2Name || "파일을 선택하세요." }}
                </span>
              </div>
    ...
    >
      <ul class="divide-y">
        <li
          v-for="(v, idx) in typePicker.list.value"
          :key="idx"
          class="p-2 flex items-center gap-3 cursor-pointer"
          @click="typePicker.selected.value = v"
        >
          <RadioButton
            :inputId="`tp_${idx}`"
            name="tp_pick"
            :value="v"
            v-model="typePicker.selected.value"
          />
          <label :for="`tp_${idx}`" class="truncate">{{ v }}</label>
        </li>
        <li
          v-if="typePicker.list.value.length === 0"
      ...
      <div class="relative mt-3 min-h-[42px]">
        ...
        <div class="flex items-center gap-2 justify-end text-[14px]">
          <Button
            label="이전"
            text
            @click="fetchTypePage(typePicker.page.value - 1)"
            :disabled="typePicker.page.value <= 1"
          />
          <span
            >{{ typePicker.page.value }} /
            {{
              Math.max(1, Math.ceil(typePicker.total.value / typePicker.size))
            }}</span
          >
          <Button
            label="다음"
            ...
            @click="fetchTypePage(typePicker.page.value + 1)"
            :disabled="
              typePicker.page.value * typePicker.size >= typePicker.total.value
            "
          />
        </div>
      </div>
    ...
    >
      <ul class="divide-y">
        <li
          v-for="(v, idx) in locPicker.list.value"
          :key="idx"
          class="p-2 flex items-center gap-3 cursor-pointer"
          @click="locPicker.selected.value = v"
        >
          <RadioButton
            :inputId="`lc_${idx}`"
            name="lc_pick"
            :value="v"
            v-model="locPicker.selected.value"
          />
          <label :for="`lc_${idx}`" class="truncate">{{ v }}</label>
        </li>
        <li
          v-if="locPicker.list.value.length === 0"
          class="p-4 text-center text-gray-500"
        >
          항목 없음
        </li>
      ...
      <div class="relative mt-3 min-h-[42px]">
        ...
        <div class="flex items-center gap-2 justify-end text-[14px]">
          <Button
            label="이전"
            text
            @click="fetchLocPage(locPicker.page.value - 1)"
            :disabled="locPicker.page.value <= 1"
          />
          <span
            >{{ locPicker.page.value }} /
            {{
              Math.max(1, Math.ceil(locPicker.total.value / locPicker.size))
            }}</span
          >
          <Button
            label="다음"
            ...
            @click="fetchLocPage(locPicker.page.value + 1)"
            :disabled="
              locPicker.page.value * locPicker.size >= locPicker.total.value
            "
          />
        </div>
      </div>
