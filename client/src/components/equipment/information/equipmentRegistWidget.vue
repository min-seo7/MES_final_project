...
const emit = defineEmits(['save', 'reset', 'open:typePicker', 'open:locPicker']);
...
function onFile1(e) {
  ...
  file1Name.value = f ? f.name : '';
}
function onFile2(e) {
  const f = e.target.files?.[0];
  file2Name.value = f ? f.name : '';
}
</script>
  ...
  <div class="mt-8 space-y-4">
    ...
    <div class="border rounded-md overflow-hidden bg-white">
      <div class="grid grid-cols-[9rem_1fr_9rem_1fr]">
        <!-- 설비코드 / 설비명 -->
        <div class="bg-gray-50 border-r border-b px-2 py-4 text-[14px] font-medium flex items-center justify-center whitespace-nowrap">
        ...
        <!-- 파일첨부 -->
        <div class="bg-gray-50 border-r px-2 py-4 text-[14px] font-medium flex items-center justify-center whitespace-nowrap">
          파일첨부
        </div>
