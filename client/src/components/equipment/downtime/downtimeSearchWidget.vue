...
<template>
  <div class="flex items-center justify-between mb-2">
    <div class="font-bold text-[18.5px]">설비 비가동 관리</div>
  ...
  <div class="flex justify-start border p-4 rounded-md bg-white">
    <div class="flex items-center gap-2">
      ...
      <div class="relative w-80">
        ...
        <i
          class="pi pi-search absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-gray-500"
          @click="openPicker"
        ></i>
      </div>
    </div>
  ...
  >
    <ul class="divide-y">
      <li
        v-for="(code, idx) in picker.list.value"
        :key="idx"
      ...
      >
        <RadioButton
          :inputId="`eq_${idx}`"
          :value="code"
          v-model="picker.selected.value"
        />
        <label :for="`eq_${idx}`">{{ code }}</label>
      </li>
      <li
        v-if="!picker.list.value.length"
    ...
    <div class="flex justify-between items-center mt-3 text-sm">
      <Button
        label="이전"
        text
        @click="fetchPage(picker.page.value - 1)"
        :disabled="picker.page.value <= 1"
      />
