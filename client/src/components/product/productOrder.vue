...
const fetchPrdOrders = async () => {
  try {
    ...
    if (Array.isArray(response.data.list)) {
      ...
      prdOrderList.value = [];
      console.warn(
        "서버 응답이 배열이 아닙니다. DataTable에 빈 배열을 할당합니다."
      );
    }
...
const selectModalValue = (value) => {
  ...
  // 제품명 모달 처리
    ...
    if (currentEditRow.value) {
      // 변경할 행의 인덱스 찾기
      const rowIndex = products.value.findIndex(
        (p) => p.id === currentEditRow.value.id
      );
...
const startProduction = async () => {
  ...
  try {
    ...
    const response = await axios.post(
      "/api/production/productionOrder",
      payload
    );
  ...
  try {
    await axios.post("/api/production/bomRequestInsert", {
      details: bomPayload,
    });
...
<template>
  ...
  <div class="mb-6">
    ...
    >
      <Column
        v-for="col of prdOrderDetailcolumns"
        :key="col.field"
        :field="col.field"
        :header="col.header"
      >
      ...
      >
        ...
        >
          ...
          <template #editor="{ data, field }">
            ...
            <template
              v-else-if="
                ['productPlanQty', 'undefinedQty', 'currentQty'].includes(field)
              "
            >
