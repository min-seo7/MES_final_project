...
<template>
  <div>
    <div class="font-semibold text-2xl mb-4">반품 입고</div>
    ...
    <div class="card w-full">
      <DataTable
        v-model:selection="selectedReturnList"
      ...
      >
        ...
        <Column header="입고수량" style="width: 10rem">
          <template #body="slotProps">
            <InputNumber
              v-model="slotProps.data.receiptQty"
              :disabled="slotProps.data.status === '확정'"
            />
          </template>
        </Column>
        ...
        <Column header="보관위치" style="width: 10rem">
          <template #body="slotProps">
            <InputText
              ...
              @click="openWarehouseeModal(slotProps.index)"
              :disabled="slotProps.data.status === '확정'"
            />
          </template>
        </Column>
