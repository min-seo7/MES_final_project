  ...
  methods: {
    ...
    async postRegistOut() {
      try {
        ...
        let prdStotalStock = await axios.post("/api/stock/checkStock", {
        ...
        for (let row of this.selectOutWaitPrds) {
          let stockInfo = prdStotalStock.data.find(
            (item) => item.product_id === row.w_prdCode
          );
    ...
    onSelecPartner() {
      this.outWaitPrds[this.selectRow].w_shipEnt =
        this.selectPartner.partnerName;
...
<template>
  <div>
    <div class="font-semibold text-2xl mb-4">제품 출고</div>
      ...
      <Tabs value="0">
        ...
        <TabPanels>
          <TabPanel value="0">
            ...
            <!--0번탭 컨텐츠영역-->
            <stockCommTable
              v-model:selection="selectOutWaitPrds"
              :columns="outWaitPrdsCol"
              :dataRows="outWaitPrds"
            />
          </TabPanel>
          <TabPanel value="1">
            ...
            <!--1번탭 컨텐츠영역-->
            <stockCommTable
              v-model:selection="selectOutPrds"
              :columns="outPrdCol"
              :dataRows="outPrds"
              :dataKey="'id'"
            />
  ...
  <commModal v-model="partnerModal" header="거래처목록" style="width: 33rem">
    ...
    <DataTable
      v-model:selection="selectPartner"
      :value="partners"
      dataKey="partnerId"
