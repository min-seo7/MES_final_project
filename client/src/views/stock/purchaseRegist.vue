...
export default {
  ...
  methods: {
    ...
    onSelectMat() {
      ...
      const isDuplicate = this.purshaseList.some(
        (item, index) =>
...
<template>
  <div>
    <div class="font-semibold text-2xl mb-4">발주 등록</div>
  </div>
          ...
          >
            ...
            >
              <template #body="slotProps">
                <InputText
                  v-model="slotProps.data.mat_id"
                  @click="openMatModal(slotProps.index)"
                  style="width: 80px"
                />
              </template>
            </Column>
