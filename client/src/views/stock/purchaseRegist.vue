...
export default {
    ...
    data() {
        return {
            ...
            materials: [
                {id: 1, matCode: 'MA001', matName: '왕겨', unit: 'KG' },
                {id: 2, matCode: 'MA002', matName: '왕겨왕겨', unit: 'EA' }
            ],
        };
    },
    methods: {
       onSelectPartner(){ //(모달)거래처선택시 반영.
            this.partnerId = this.selectPartner.partnerId;
            this.partnerName = this.selectPartner.partnerName;

              this.purshaseList.forEach(row => { //배열의 각 행을 순환하면서 거래처명.  
                 row.patner = this.partnerName;
              });

            this.partnerModal = false;
        },
        openMatModal(rowIndex){ //자재코드 모달클릭시 index번호 추출
            this.selectRow = rowIndex;
            this.materialModal = true;
        },
        onSelectMat(){ //(모달)자재선택시 반영
        ...
        inceaseCount(){    //발주목록 행추가시 목록번호 증가. 
            return ++this.count;
    },
    },
    mounted() {
...
<!--공급처모달-->
  <commModal v-model="partnerModal" header="공급처목록">
    ...
    <DataTable v-model:selection="selectPartner" :value="partners" dataKey="id" tableStyle="min-width: 50rem"> 
    ...
    <!-- footer 슬롯 -->
    <template #footer> <!--commModal 컴포넌트 안의 <slot name="footer"> 영역에 여기서 작성한 버튼 내용을 삽입하겠다는 의미-->
      <Button label="선택 완료" @click="onSelectPartner()" />
    </template>
  </commModal>

<!--자재모달-->
  <commModal v-model="materialModal" :value="materials" header="자재목록">
  <div class="">
