...
const onlyFilled = (rows) => (rows || []).filter((r) => r?.item?.trim() || '' || r?.result?.trim() || '' || r?.action?.trim() || '');
...
<template>
    <div class="p-4 space-y-6">
        ...
        <inspectionRegistWidget v-model="form" :isEdit="isEdit" @save="handleSave" @update="handleUpdate" @reset="handleReset" @open:eqPicker="openEqPicker" />
    </div>
</template>
