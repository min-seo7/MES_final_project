...
function handleSearch(q) {
  ...
  const match = pickerData.find(s =>
    (!q.eq_id   || s.eq_id   === q.eq_id) &&
    (!q.eq_type || s.eq_type === q.eq_type) &&
    (!q.eq_name || s.eq_name === q.eq_name) &&
    (!q.loc     || s.loc     === q.loc)
  );
...
// 저장/초기화 (등록 위젯용)
function saveForm()  { console.log('저장 데이터:', form.value); }
function resetForm() {
  form.value = {
    equipmentCode: '',
    equipmentType: '',
