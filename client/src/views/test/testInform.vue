...
const registItem = async () => {
  try {
    if (
      ...
      !selectItem.value.inspPurpose ||
      !selectItem.value.inspItem ||
    ...
    if (payload.rangeOperator && payload.rangeValue)
      payload.fixedStandard = `${payload.rangeOperator} ${payload.rangeValue}`;
    ...
    if (!payload.purposeid) {
      const found = inspTypeList.value.find(
        (x) => x.Type === payload.inspPurpose
      );
...
onMounted(async () => {
  try {
    const [pRes, ipRes, itRes, oRes, uRes] = await Promise.all([
      axios.get("/api/test/productTypes"),
      axios.get("/api/test/inspPurposes"),
      axios.get("/api/test/inspItems"),
      axios.get("/api/test/operators"),
      axios.get("/api/test/units"),
    ]);
      ...
      <template #center>
        <div class="flex items-center gap-4 text-xs">
          <div class="flex items-center gap-1">
            ...
            <div
              class="flex items-center border rounded cursor-pointer hover:bg-gray-100 px-2 h-[28px]"
            >
              <div
          ...
          <div class="flex items-center gap-1">
            ...
            >
              <div
                v-for="item in inspTypeList"
                :key="item.code"
                class="flex items-center gap-1 mr-2"
              >
                <RadioButton
                  :inputId="'search_ip_' + item.code"
                  name="searchInspPurpose"
                  v-model="search.inspPurpose"
                  :value="item.Type"
                />
                <label :for="'search_ip_' + item.code">{{ item.Type }}</label>
              </div>
              <div
                v-if="inspTypeList.length === 0"
                class="text-xs text-gray-500"
              >
                데이터 없음
              </div>
          ...
          <Column header="삭제" style="width: 80px" class="text-center">
            <template #body="{ data }">
              <Button
                ...
                @click="deleteItem(data.testitem_code)"
              />
            </template>
          </Column>
        </DataTable>
      ...
      <template #content>
        ...
        >
          <div>
            ...
            <InputText
              v-model="selectItem.testitem_code"
              readonly
              class="bg-gray-100 w-full text-sm"
            />
          </div>
          ...
          <div class="md:col-span-1 lg:col-span-1">
            ...
            <div class="flex flex-col space-y-1 p-1 border rounded">
              ...
              >
                <RadioButton
                  :inputId="'reg_pt_' + item.code"
                  name="regProductType"
                  v-model="selectItem.productType"
                  :value="item.Type"
                />
                <label :for="'reg_pt_' + item.code" class="text-sm">{{
                  item.Type
                }}</label>
              </div>
              <div
          ...
          <div>
            ...
            <div class="relative flex items-center">
              <InputText
                readonly
                v-model="selectItem.inspPurpose"
                class="w-full cursor-pointer bg-white pr-8 text-sm"
                @click="openModal('inspPurpose', selectItem)"
              />
              <i
                class="pi pi-search absolute right-2 text-gray-500 cursor-pointer"
                @click="openModal('inspPurpose', selectItem)"
              ></i>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="relative flex items-center">
              <InputText
                readonly
                v-model="selectItem.inspItem"
                class="w-full cursor-pointer bg-white pr-8 text-sm"
                @click="openModal('inspItem', selectItem)"
              />
              <i
                class="pi pi-search absolute right-2 text-gray-500 cursor-pointer"
                @click="openModal('inspItem', selectItem)"
          ...
          <div>
            ...
            <div class="relative flex items-center">
              <InputText
                readonly
                v-model="selectItem.unit"
                class="w-full cursor-pointer bg-white pr-8 text-sm"
                @click="openModal('unit', selectItem)"
              />
              <i
                class="pi pi-search absolute right-2 text-gray-500 cursor-pointer"
                @click="openModal('unit', selectItem)"
          ...
          <div class="col-span-1 lg:col-span-2">
            ...
            <div class="flex gap-2 items-center">
              <div class="relative w-1/3 flex items-center">
                <InputText
                  readonly
                  v-model="selectItem.rangeOperator"
                  class="cursor-pointer w-full bg-white pr-8 text-sm"
                  @click="openModal('rangeOperator', selectItem)"
                />
                <i
                  class="pi pi-search absolute right-2 text-gray-500 cursor-pointer"
                  @click="openModal('rangeOperator', selectItem)"
      ...
      <div>
        ...
        <div v-if="modalList && modalList.length > 0">
          ...
          >
            ...
            <Column
              field="Type"
              :header="modalType === 'rangeOperator' ? '연산자' : '항목'"
              class="text-center"
            >
              <template #body="{ data }">
                <button
                  class="text-center w-full py-1 hover:bg-gray-100"
                  @click="selectModalValue(data)"
                >
                  {{ data.Type }}
                </button>
              </template>
