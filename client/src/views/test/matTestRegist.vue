            ...
            >
              ...
              <div class="flex gap-2">
                <div class="flex items-center gap-1">
                  <RadioButton
                    v-model="item.판정"
                    :inputId="`check_clear_${index}`"
                    :name="`checkval_${index}`"
                    value="적합"
                  />
                  <label :for="`check_clear_${index}`" class="text-sm"
                    >적합</label
                  >
                </div>
                <div class="flex items-center gap-1">
                  <RadioButton
                    v-model="item.판정"
                    :inputId="`check_fail_${index}`"
                    :name="`checkval_${index}`"
                    value="부적합"
                  />
                  <label :for="`check_fail_${index}`" class="text-sm"
                    >부적합</label
                  >
                </div>
                <InputText
                  v-model="item.판정"
                  readonly
                  class="ml-auto w-1/2 text-sm text-red-600 font-bold"
                />
              </div>
            </div>
