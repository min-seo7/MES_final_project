...
<template>
    <div class="p-4">
        <div class="flex items-center justify-between text-xl mb-4">
            <h4>제품 품질 검사 결과</h4>
            <div class="space-x-1">
                <Button label="조회" class="text-xs px-2 py-1 h-[28px]" rounded @click="fetchPrdInspFinList" />
            ...
            <template #center>
                <div class="flex items-center gap-4 text-sm">
                    <div class="flex items-center gap-2">
                        ...
                        <InputText id="perfCode" type="text" v-model="filterPerfCode" placeholder="검색" class="h-8 text-sm" />
                    </div>
                    <div class="flex items-center gap-2">
                        <label for="productCode" class="whitespace-nowrap">제품코드</label>
                        <InputText id="productCode" type="text" v-model="filterProductCode" placeholder="검색" class="h-8 text-sm" />
                    </div>
                </div>
            </template>
        </Toolbar>
        ...
        <div class="card p-4">
            ...
            <DataTable :value="prdInspecList" selectionMode="single" dataKey="_key" @rowSelect="handleRowSelect" v-model:selection="selectedItem" :paginator="true" :rows="6" class="compact-table">
        ...
        <Dialog v-model:visible="showDetail" header="검사 성적서" modal style="width: 700px">
                ...
                <table class="border-collapse w-full text-sm">
                    <tbody>
                        <tr>
                            ...
                            <td class="border p-2" colspan="3">{{ selectedItem?.검사결과 }}</td>
                        </tr>
                        <tr>
                ...
                <table class="border-collapse w-full text-sm mt-4">
                    <thead>
                        <tr>
                            ...
                            <th class="border p-2 bg-gray-100 font-medium">기준치/허용범위</th>
            ...
            <div class="flex justify-end mt-4">
                <Button label="닫기" class="text-xs px-2 py-1 h-[28px]" rounded @click="showDetail = false" />
            </div>
        </Dialog>
    </div>
</template>
