...
<template>
    <div class="font-semibold text-2xl mb-4">제품 품질 검사 결과</div>
    <div>
    ...
    <div class="card flex flex-col gap-4">
        ...
        <DataTable :value="customers2" selectionMode="single" dataKey="name" @rowSelect="handleRowSelect" :selection="selectedItem" @update:selection="(val) => (selectedItem = val)">
            <Column field="제품검수번호" header="제품검수번호" style="min-width: 80px" frozen class="font-bold" />
            ...
            <table class="border-collapse border w-full text-sm">
                <tbody>
                    <tr>
                        ...
                        <td class="border p-2 w-1/4">{{ selectedItem?.제품검수번호 }}</td>
                    </tr>
                    <tr>
