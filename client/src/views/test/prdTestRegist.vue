...
const defRegister = async () => {
  ...
  const isAllMeasured = form.value.측정값리스트.every(
    (item) => item.측정값 !== "" && item.측정값 !== null
  );
  if (!isAllMeasured) {
  ...
  const isFailed = form.value.측정값리스트.some(
    (item) => item.판정 === "부적합"
  );
    ...
    <div class="flex flex-col md:flex-row gap-6">
      ...
      <div class="md:w-1/3">
        <div class="card p-4">
          ...
          <div
            v-for="(item, index) in form.측정값리스트"
            :key="index"
            class="mb-2"
          >
            ...
            <div class="flex gap-2 items-end">
              <div class="flex-1">
                ...
                <InputText
                  v-model="item.허용범위"
                  readonly
                  class="w-full text-xs bg-gray-100"
                />
              </div>
              <div class="flex-1">
              ...
              <div class="flex-1">
                <label class="block text-xs text-red-500 font-semibold"
                  >판정</label
                >
                <InputText
                  v-model="item.판정"
                  readonly
                  class="w-full text-xs bg-gray-100 text-red-600 font-bold"
                />
              </div>
            </div>
          </div>
