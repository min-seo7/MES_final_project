...
router.get("/selectProcessList", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "공정 목록 조회 실패", error: error.message });
  }
});
// 페이지 - 공정 리스트
router.get("/processesNotSearchList", async (req, res) => {
  try {
    const selectProcessNotSearchList =
      await productionService.selectProcessNotSearchList();
...
router.get("/productListModal", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "제품 목록 조회 실패", error: error.message });
  }
});
...
router.get("/fetchPlanList", async (req, res) => {
  try {
    const planList = await productionService.fetchPlanList();
    res.status(200).json({ list: planList });
  } catch (error) {
    console.error("제품 목록 조회 실패:", error);
    res
      .status(500)
