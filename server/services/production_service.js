...
const insertPlan = async (payload) =>{
  ...
  try {
    ...
    if (lastPlanNo) {
      ...
      masterPlanId = `PNO${new Date().toISOString().slice(0,10).replace(/-/g,'')}-${String(numPart + 1).padStart(5,'0')}`;
    } else {
      masterPlanId = `PNO${new Date().toISOString().slice(0,10).replace(/-/g,'')}-00001`;
    }
